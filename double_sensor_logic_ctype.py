...
def set_tdt_values():
    ...
    try:
        ...
        while datetime.now() < stop_time:
            entry_sensor_reading = vl53.read_distance(0)
            exit_sensor_reading = vl53.read_distance(1)
...
def set_baseline_values():
    ...
    entry_sensor_baseline = vl53.read_distance(0)
    exit_sensor_baseline = vl53.read_distance(1)
...
def main():
    ...
    if args.calibrate:
        ...
        while user_reply not in ['y', 'yes', 'n', 'no']:
            user_reply = input('Please enter y or n. Press Ctrl+C to exit.\n').strip().lower()
        if user_reply in ['y', 'yes']:
            min_tdt, max_tdt = set_tdt_values()
            entry_sensor_baseline, exit_sensor_baseline = set_baseline_values()
